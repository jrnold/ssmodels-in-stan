...
vector[] dlm_constant_filter(int n,
                             int r, int p,
                             vector[] y,
                             int[,] miss,
                             vector b,
  ...
  for (t in 1:n) {
    ...
    for (j in 1:r) {
      if (int_step(miss[t, j])) {
        ...
        m <- m + K * err;
        C <- make_symmetric_matrix(C - K * Q * K ');
        loglik[t, j] <- - 0.5 * (log(2 * pi()) + log(Q) + err ^ 2 * Q_inv);
      }
    }
  }
  return loglik;
...
vector dlm_local_level_loglik(int n,
  ...
  for (t in 1:n) {
    ...
    if (int_step(miss[t])) {
      ...
      m <- m + K * err;
      C <- C - pow(K, 2) *  Q;
      loglik[t] <- - 0.5 * (log(2 * pi()) + log(Q) + pow(err, 2) * Q_inv);
    }
  }
  return loglik;
}
