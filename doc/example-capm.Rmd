...
capm_mod <- ssm_stan_model("../StanStateSpace/inst/stan/models/var_only_mv.stan")
```
```{r}
capm_data <-
  within(list(), {
...
```{r capm-state-plot}
capm_betas <- filter(capm_summary, parameter == "beta", dim_1 > 1) %>%
...
capm_beta_mean <- filter(capm_summary, parameter == "beta_mean", dim_1 > 1) %>%
         mutate(stock = recode(dim_2,
                                `1` = "mobil",
                                `2` = "ibm",
                                `3` = "weyer",
...
ggplot(capm_betas_pred, aes(x = date, y = mean)) +
  geom_line() +
  theme_minimal()
