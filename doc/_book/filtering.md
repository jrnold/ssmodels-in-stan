...
Write the univariate representation of this model as,
$$
...
\alpha_{t + 1, 1} &= \vec{c}_t + \mat{T}_t \alpha_t, & t &= 1, \dotas, n
\end{aligned}
$$

Define the predicted and filtered expected values and variances of the states as
$$
\begin{aligned}[t]
...
The univariate series is
$$
y_{1,1}, \dots, y_{1,p_1}, y_{2,1}, \dots, y_{n - 1, p_{n -1}}, y_{n,1}, \dots, y_{n, p_n} .
$$
With this univariate series the filtering update steps as each $y_{t,i}$ is processed are, for $i = 1, \dots, p_t$,
$$
\begin{aligned}[t]
...
\end{aligned}
$$
The prediction update step to transition from time $t$ to $t + 1$ is,
$$
\begin{aligned}[t]
...
\end{aligned}
$$
The values of $\vec{a}_{t + 1, 1}$ and $\mat{P}_{t + 1, 1}$ are the same as the
values of $\vec{a}_{t + 1}$ and $\mat{P}_{t + 1}$ in the standard Kalman filter.
However, the values of $v_{t,i}$, $\vec{K}_{t,i}$, and $F_{t,i}^{-1}$ are nnot the same.

**smoothing**, state smoothing, and observation disturbances.
