...
- `matrix`  **x** A $p \times q$, $p \leq m$, $\q \leq n$ matrix
- `int`  **m** Number of rows in the returned matrix
- `int`  **n** Number of columns in the returned matrix
- `int`  **i** Indices mapping the rows of $A$ to the rows in the output matrix
- `int`  **j** Indices mapping the columns of $A$ to the columns of the output matrix
- `real`  **a** The default value in the returned matrix

**Return Value:** `matrix` A $m \times n$ matrix
...
- `vector`  **x** A $p \times q$, $p \leq m$, $\q \leq n$ matrix
- `int`  **n** Number of elements in the returned vector
- `int`  **i** Indices mapping the rows of $A$ to the rows in the output matrix
- `real`  **y** The default value in the returned vector

**Return Value:** `vector` A $n \times 1$ matrix
...
- `int[]`  **x** An array of length $n$ of integers

**Return Value:** `int` An integer between 0 and $n$.
...
- `int[]`  **x** An array of length $n$ of integers
- `int`  **m** The number of false values in `x`.

**Return Value:** `int[]` An array of integers with elements having values between 1 and $m$.
...
- `int[]`  **x** An array of length $m$ of integers
- `int`  **n** The number of true values in `x`.
...
**Return Value:** `real` A value drawn from the specified normal distribution.
...
**Return Value:** `matix` An $n \times n$ lower-triangular matrix
...
- `vector`  **mu** An $n \times 1$ vector of the means
- `matrix`  **Sigma** An $n \times n$ lower triangular matrix with covariance matrix.

**Return Value:** `vector` An $n \times 1$ vector drawn from the specified multivariate normal distribution.
...
- `vector`  **mu** An $n \times 1$ vector of the means
- `matrix`  **L** An $n \times n$ lower triangular matrix with the Cholesky decomposition of the covariance matrix.
...
- `vector`  **a** An $m \times 1$ vector with the predicted state, $\vec{a}_t$.
...
- `matrix`  **P** An $m \times m$ vector with the variance of the predicted state, $\mat{P}_t$.
...
- `matrix`  **y** A $p \times 1$ vector of the observations, $\vec{y}_t$.
- `vector`  **a** A $m \times 1$ vector of the states, $\vec{a}_t$.
- `matrix`  **d** A $p \times 1$ vector with the observation intercept, $\vec{d}_t$.
- `matrix`  **Z** A $p \times m$ matrix with the design matrix, $\mat{Z}_t$.

**Return Value:** `vector` A $p \times 1$ vector of the forecast errors, $\vec{v}_t$.
...
- `matrix`  **d** A $p \times 1$ vector with the observation intercept, $\vec{d}_t$.
- `matrix`  **Z** A $p \times m$ matrix with the design matrix, $\mat{Z}_t$.
- `int[]`  **y_idx** A length $p$ array of integers indexes with the indexes of thenon-missing values of $y$. Elements $1:p_t$ should be between $1$ and $p$; elements $p_t:p$ are zero, and are not used.
...
- `matrix`  **Z** A $p \times m$ matrix with the design matrix, $\mat{Z}_t$.
- `matrix`  **H** A $p \times p$ matrix with the observation covariance matrix, $\mat{H}_t$.
- `int`  **p** The number of non-missing observations in $\vec{y}_t$.
- `int[]`  **y_idx** A length $p$ array of integers. The first $p_t$ elments of this array indexes of thenon-missing values of $y$. Elements $1:p_t$ should be between $1$ and $p$; elements $p_t:p$ are zero, and are not used.
...
- `matrix`  **P** An $m \times m$ vector with the variance of the predicted state, $P_t$.
- `matrix`  **Z** A $p \times m$ matrix with the design matrix, $\mat{Z}_t$.
- `matrix`  **T** An $m \times m$ matrix with the transition matrix, $\mat{T}_t$.
...
**Return Value:** `real` The log-likelihood
...
- `vector`  **v** A $p \times 1$ matrix with the forecast error, $\vec{v}_t$.
- `matrix`  **Finv** A $p \times p$ matrix with variance of the forecast error, $\mat{F}^{-1}_t$.
- `int`  **p_t** The number of non-missing observations in $\vec{y}_t$.
- `int[]`  **y_idx** A length $p$ array of integers with the indexes of the nom-missing values in $\vec{y}_t$. Elements $1:p_t$ should be values between $1$ and $p$; elements $p_t:p$ are zero and are not used.
...
- `vector`  **a** An $m \times 1$ vector with the expected value of the predicted state, $\vec{a}_t$.
- `matrix`  **P** An $m \times m$ vector with the variance of the predicted state, $\mat{P}_t$.
- `matrix`  **Z** A $p \times m$ matrix with the design matrix, $\mat{Z}_t$.
- `vector`  **v** A $p \times 1$ vector with the forecast errors, $\vec{v}_t$
- `matrix`  **Finv** A $p \times p$ matrix with the forecast prediction, $\mat{F}_{t}^{-1}$.

**Return Value:** `matrix` An $m \times 1$ matrix the expected value of the fitered states, $\E(\vec{alpha}_t | \vec{y}_{1:t}) = \vec{a}_{t|t}$.
...
**Return Value:** `matrix` An $m \times m$ matrix with variance of the filtered states, $\Var(\vec{alpha}_t | \vec{y}_{1:t}) = \mat{P}_{t|t}$.
...
**Return Value:** `real` If converged, then 1, else 0.
...
- `vector[]`  **filter** A length $n$ array with results from `ssm_filter`.
- `vector[]`  **d** Observation intercept, $\vec{d}_t$. An array of $p \times 1$ vectors.
- `matrix[]`  **Z** Design matrix, $\mat{Z}_t$. An array of $p \times m$ matrices.
- `matrix[]`  **H** Observation covariance matrix, $\mat{H}_t$. An array of $p \times p$ matrices.
- `vector[]`  **c** State intercept, $\vec{c}_t$. An array of $m \times 1$ vectors.
