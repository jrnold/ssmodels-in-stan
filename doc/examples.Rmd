...
```{r Nile-Model}
local_level_code <- stanc_builder(find_rstudio_root_file("stan/local_level.stan"))
# rm(local_level_mod)
# local_level_mod <- stan_model(model_code = local_level_code[["model_code"]],
#                               model_name = local_level_code[["model_namze"]])
...
nile_init <- list(list(sigma_epsilon = array(sqrt(var(nile_data$y)), 1),
                       sigma_eta = array(sqrt(var(nile_data$y)), 1)))
...
```{r Nile-predicted-states}
nile_params <- 
  extract_param_from_ssm_filter(rstan::extract(nile_ll_sample, "filter_res")[[1]],
                                1, 1) 

nile_states <- nile_params[["a"]] %>%
  apply(2:3, mean)
ggplot(mutate(Nile_, state = nile_states), aes(x = year)) +
  geom_point(aes(y = flow)) +
  geom_line(aes(y = state))
```

```{r Nile-smoothed-states}
nile_smooth <- apply(drop(rstan::extract(nile_ll_sample, "alpha2")[[1]]), 2, mean)
ggplot(mutate(Nile_, state = nile_smooth), aes(x = year)) +
  geom_point(aes(y = flow)) +
  geom_line(aes(y = state))
```
