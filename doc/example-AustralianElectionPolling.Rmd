...
```{r AustralianElectionPolling-data}
library("dplyr")
data("AustralianElectionPolling", package = "pscl")
glimpse(AustralianElectionPolling)
```
...
```{r}
polling_mod <- ssm_stan_model("stan/polling.stan")
```
...
```{r}
states <- filter(polling_samples, parameter == "alpha")

ggplot() +
  geom_ribbon(data = states,
              aes(x = date, ymin = mean - 2 * sd, ymax = mean + 2 * sd), alpha = 0.3) +
  geom_point(data = aus_polling, aes(x = date, y = y, colour = org), alpha = 0.6) +  
