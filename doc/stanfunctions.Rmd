...
**Return Value:** `An` $n times n$ symmetric matrix: $0.5 (x + x')$.
...
- `vector`  **a** An $m \times 1$ vector with the prected state, $a_t$.
- `vector`  **c** An $m \times 1$ vector with the system intercept, $c_t$
- `matrix`  **T** An $m \times m$ matrix with the transition matrix, $T_t$.
- `vector`  **v** A $p \times 1$ vector with the forecast error, $v_t$.
- `matrix`  **K** An $m \times p$ matrix with the Kalman gain, $K_t$.

**Return Value:** `vector` A $m \times 1$ vector with the predicted state at $t + 1$, $a_{t + 1}$.
...
- `matrix`  **Z** A $p \times m$ matrix with the design matrix, $Z_t$.
- `matrix`  **T** An $m \times m$ matrix with the transition matrix, $T_t$.
- `matrix`  **RQR** A $m \times m$ matrix with the system covariance matrix, $R_t Q_t R_t'$.
- `matrix`  **K** An $m \times p$ matrix with the Kalman gain, $K_t$.

**Return Value:** `matrix` An $m \times 1$ vector with the predicted state at $t + 1$, $a_{t + 1}$.
...
- `matrix`  **Z** A $p \times m$ matrix with the design matrix, $Z_t$.
- `matrix`  **T** An $m \times m$ matrix with the transition matrix, $T_t$.
- `matrix`  **RQR** An $m \times m$ matrix with the system covariance matrix, $R_t Q_t R_t'$.
- `matrix`  **K** An $m \times p$ matrix with the Kalman gain, $K_t$.

**Return Value:** `vector` An $m \times 1$ vector with the predicted state at $t + 1$, $a_{t + 1}$.
...
- `matrix`  **P** An $m \times m$ vector with the variance of the prected state, $P_t$.
- `matrix`  **Z** A $p \times m$ matrix with the design matrix, $Z_t$.
- `matrix`  **H** A $p \times p$ matrix with the observation covariance matrix, $H_t$.
...
**Return Value:** `matrix` An $m \times p$ matrix with the Kalman gain, $K_t$.
...
**Return Value:** `matrix` An $m \times m$ matrix, $L_t$.
...
- `vector`  **v** A $p \times 1$ matrix with the forecast error, $v_t$.
- `matrix`  **Finv** A $p \times p$ matrix with variance of the forecast error, $F^{-1}_t$.
...
**Return Value:** `real` The log-likelihood $L_t$
...
**Return Value:** `vector` A $p \times 1$ vector with the forecast error, $v_t$.
...
**Return Value:** `vector` An $m \times 1$ vector with the expected value of the predicted state, $\E(\vec{alpha}_t | \vec{y}_{1:(t-1)} = a_t$.
...
**Return Value:** `matrix` An $m \times m$ matrix with the variance of the predicted state, $\Var(\vec{alpha}_t | \vec{y}_{1:(t-1)} = P_t$.
...
**Return Value:** `matrix` An $m \times 1$ vector with the filtered expected value of the staate,
...
**Return Value:** `matrix` An $m \times m$ matrix with the filtered variance of the staate,
...
**Return Value:** `vector[]` An array of vectors constaining $\hat{\vec{varepsilon}}_t$ and $\Var(\vec{\varepsilon}_t | \vec{y}_{1:n})$
...
**Return Value:** `vector[]` An array of size $n$ of $m \times 1$ vectors containing $\hat{\vec{\alph}}_t$.
...
- `vector`  **x** An unconstrained vector in (-Inf, Inf)
