...
## Nile with intervention (intercept)

```{r}
local_level_reg_code <-
  stanc_builder(file.path(STAN_SSM_MODELS, "local_level_reg.stan"),
                isystem = STAN_SSM_INCLUDE)
local_level_reg_mod <-
  stan_model(model_code = local_level_reg_code[["model_code"]],
                              model_name = local_level_reg_code[["model_name"]])
```



## local level with known intervention

```{r}
local_level_interven_code <-
  stanc_builder(file.path(STAN_SSM_MODELS, "local_level_interven.stan"),
                isystem = STAN_SSM_INCLUDE)
local_level_inverven_mod <-
  stan_model(model_code = local_level_interven_code[["model_code"]],
             model_name = local_level_interven_code[["model_name"]])
```

## local level with time-varying variances

```{r}
local_level_tvvar_code <-
  stanc_builder(file.path(STAN_SSM_MODELS, "local_level_tvvar.stan"),
                isystem = STAN_SSM_INCLUDE)
local_level_tvvar_mod <-
  stan_model(model_code = local_level_tvvar_code[["model_code"]],
             model_name = local_level_tvvar_code[["model_name"]])
```
