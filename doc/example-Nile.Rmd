...
nile_1_states2 <-
  ...
  mutate(par_id = rownames(nile_1_summary[["summary"]])) %>%
  inner_join(
    `colnames<-`(as_data_frame(str_match(str_keep(rownames(nile_1_summary[["summary"]]), "^alphar\\["), "^alphar\\[(\\d+),(\\d+)\\]")), c("par_id", "time", "index")),
    by = "par_id"
  ) %>%
  mutate(time = as.integer(time), index = as.integer(index))

ggplot(dplyr::filter(nile_1_states2, index == 1),
       aes(x = time, y = mean, 
           ymin = mean - 2 * sd,
           ymax = mean + 2 * sd)) +
  geom_ribbon(alpha = 0.3) +
...
nile_1_eps <-
  ...
  mutate(par_id = rownames(nile_1_summary[["summary"]])) %>%
  inner_join(
    `colnames<-`(as_data_frame(str_match(str_keep(rownames(nile_1_summary[["summary"]]), "^epsr\\["), "^epsr\\[(\\d+),(\\d+)\\]")), c("par_id", "time", "index")),
    by = "par_id"
  ) %>%
  mutate(time = as.integer(time), index = as.integer(index))
...
nile_1_eta <-
  ...
  mutate(par_id = rownames(nile_1_summary[["summary"]])) %>%
  inner_join(
    `colnames<-`(as_data_frame(str_match(str_keep(rownames(nile_1_summary[["summary"]]), "^etar\\["), "^etar\\[(\\d+),(\\d+)\\]")), c("par_id", "time", "index")),
    by = "par_id"
  ) %>%
  mutate(time = as.integer(time), index = as.integer(index))
